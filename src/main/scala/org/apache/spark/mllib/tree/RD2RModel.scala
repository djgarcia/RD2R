...
class RD2RModel(val nTrees: Int, val disc: Array[Array[Array[Double]]], val pcaList: Array[PCAModel], val models: Array[PipelineModel], val labelsInd: Array[String]) extends Serializable {
  ...
  def predict(test: RDD[LabeledPoint]): Array[Double] = {
    ...
    for (c <- 0 until nTrees) {
      ...
      val RD2R = pcaData.zip(featDisc).map(l => LabeledPoint(l._1.label, Vectors.dense(l._2 ++ l._1.features.toArray))).toDF()

      val predictions = models(c).transform(RD2R).select("probability").collect()
      var i = 0
      predictions.foreach {
  ...
  def predict(data: Vector): Double = {
    ...
    for (c <- 0 until nTrees) {
      ...
      val RD2R = Vectors.dense(featDisc.toArray ++ pcaData.toArray)

      val test = SparkContext.getOrCreate().parallelize(Seq(LabeledPoint(0, RD2R))).toDF()
