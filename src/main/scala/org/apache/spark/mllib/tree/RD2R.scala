...
class RD2R private(val data: RDD[LabeledPoint], val nTrees: Int, val cuts: Int) extends Serializable {
  ...
  def runTrain(): RD2RModel = {
    ...
    for (c <- 0 until nTrees) {
      ...
      //RD2R
      val RD2R = pcaData.zip(discData).map(l => LabeledPoint(l._1.label, Vectors.dense(l._2.features.toArray ++ l._1.features.toArray))).toDF()
      ...
      models(c) = pipeline.fit(RD2R)
    }
    new RD2RModel(nTrees, disc, pcaList, models, labelsInd)
  }
}

object RD2R {
  def train(input: RDD[LabeledPoint], nTrees: Int, cuts: Int) = {
    new RD2R(input, nTrees, cuts).runTrain()
  }
}
